...
func (c callerHook) Fire(entry *logrus.Entry) error {
	...
	once.Do(func() {
		...
		selfPkgName = path.Dir(fn.Name()) + "/log"
	})

	var pcs = make([]uintptr, 10)
	depts := runtime.Callers(5, pcs)
	frames := runtime.CallersFrames(pcs[:depts])
	for f, ok := frames.Next(); ok; f, ok = frames.Next() {
		//fmt.Println(f.Function, "------------", selfPkgName)
		if !strings.Contains(f.Function, selfPkgName) && !strings.Contains(f.Function, "logrus") {
			entry.Data["func"] = path.Base(f.Function)
